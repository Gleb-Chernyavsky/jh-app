...
export default class AmountOfGoods extends mixins(JhiDataUtils, AlertMixin) {
  ...
  public mounted(): void {
    this.retrieveAllAmountOfGoodss();
  }
  ...
  public retrieveAllAmountOfGoodss(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.amountOfGoods.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IAmountOfGoods): void {
  ...
  public removeAmountOfGoods(): void {
    this.amountOfGoodsService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A AmountOfGoods is deleted with identifier ' + this.removeId;
